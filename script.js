//Definimos los parámetros de la distribución normal truncada(se extiende hasta 0)
const media = 6.5;
const varianza = 9;
const desviacionEstandar = Math.sqrt(varianza);
const a = 0, b = 24;

//Funcion para calcular la distribución truncada
...
//Necesitamos 1000 valores igualmente espaciados para graficar la PDF (funcion de densidad de probabilidad)
...
function plotInitialGraphs() {
    ...
    //El área a la derecha de la media debe quedar coloreada
    const fillArea = {
        x: x.filter((xi) => xi >= media),
        y: pdf.filter((_, i) => x[i] >= media),
        type: 'scatter',
        mode: 'lines',
        line: { color: 'rgba(255, 0, 0, 0.5)' }, // Color suave para que no moleste
        fill: 'tozeroy',//Que llegue hasta abajo
    ...
    //Configuracion inicial del gráfico de muestra
    const layoutMuestra = {
...
function actualizarGraficos(tamanoMuestra, mediaMuestral) {
    //Generar la muestra aleatoria de la distribución truncada
    ...
    //Armar el histograma de muestra
    const histData = {
        x: muestra,
        type: 'histogram',
        nbinsx: 40,
    ...
    const layoutMuestra = {
        title: {
            ...
            font: {
                size: 14 //Para regular el tamano del titulo
            }
        },
        xaxis: { title: 'Horas de descanso' },
        yaxis: { title: 'Frecuencia relativa' },
    ...
    //Calcular el puntaje z y la probabilidad
    ...
    //Los textos de probabilidad y z-score
    ...
    //Gráfico de la muestra actualizada
    Plotly.react('grafico-muestra', [histData], layoutMuestra);

    //Actualizar la linea de la media en el gráfico de distribución
    const mediaLine = {
    ...
    //Actualizar el área a la derecha de la media en el gráfico de distribución
    const fillArea = {
    ...
    //Trazar la distribución truncada
    const distribucionTrace = {
        x: x,
        y: pdf,
        mode: 'lines',
    ...
    //Cnfigurar el gráfico de distribución
    const layoutDistribucion = {
        title: 'Distribución Normal Truncada',
        xaxis: { title: 'Horas de descanso' },
        yaxis: { title: 'Densidad de probabilidad' },
    ...
    //Actualizar el grafico de distribución con la nueva linea de media y area sombreada
...
function initializeSliders() {
    ...
    //Escuchar los sliders para actualizar gráficos
